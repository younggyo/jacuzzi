...
module.exports = /** @lends Balancer */ {
   ...
   * @param {Pool} pool
   */
  remove: function(pool) {
    var pools = this.pools[pool.priority]
    if (!pools) {
      return 0
    }
    ...
    if (pools.length === 0) {
      index = this.priorities.indexOf(pool.priority)
      //remove priority from priorities
      this.priorities.splice(index, 1)
    }

    return pool.pending.length
  },
